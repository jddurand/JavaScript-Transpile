 ...
 *	   to determine k = fdlibm_floor(fdlibm_log10(d)).  We scale relevant
	...
	int float_type)
{
  ...
  int bbits, b2, b5, be, dig, i, ieps, ilim0, j, fdlibm_j1, k, k0,
  ...
  if ((i = (int) (word0 (d) >> Exp_shift1 & (Exp_mask >> Exp_shift1))))
	...
	d2.d /= 1 << j;
      ...
      /* fdlibm_log(x)	~=~ fdlibm_log(1.5) + (x-1.5)/1.5
		 * fdlibm_log10(x)	 =  fdlibm_log(x) / fdlibm_log(10)
		 *		~=~ fdlibm_log(1.5)/fdlibm_log(10) + (x-1.5)/(1.5*fdlibm_log(10))
		 * fdlibm_log10(d) = (i-Bias)*fdlibm_log(2)/fdlibm_log(10) + fdlibm_log10(d2)
		 *
		 * This suggests computing an approximation k to fdlibm_log10(d) by
		 *
		 ...
		 * (We could get a more accurate k by invoking fdlibm_log10,
  ...
  if (ilim >= 0 && ilim <= Quick_max && try_quick)
	...
	{
	  d.d *= tens[fdlibm_j1 & 0xf];
	  for (j = fdlibm_j1 >> 4; j; j >>= 1, i++)
	    if (j & 1)
	      {
		ieps++;
		d.d *= bigtens[i];
  ...
  if (leftright)
      ...
      for (i = 1;; i++)
	{
	  ...
	  fdlibm_j1 = delta->_sign ? 1 : cmp (b, delta);
	  Bfree (ptr, delta);
#ifndef ROUND_BIASED
	  if (fdlibm_j1 == 0 && !mode && !(word1 (d) & 1))
	    {
	      if (dig == '9')
		goto round_9_up;
	      if (j > 0)
	  ...
	  if (j < 0 || (j == 0 && !mode
	      ...
	      if (fdlibm_j1 > 0)
		{
		  b = lshift (ptr, b, 1);
		  fdlibm_j1 = cmp (b, S);
		  if ((fdlibm_j1 > 0 || (fdlibm_j1 == 0 && dig & 1))
		      && dig++ == '9')
		    goto round_9_up;
		}
	      *s++ = dig;
