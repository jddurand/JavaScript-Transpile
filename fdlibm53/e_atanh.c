...
#ifdef __STDC__
	double __fdlibm_ieee754_atanh(double x)
#else
	double __fdlibm_ieee754_atanh(x)
	double x;
#endif
{
	double t;
	int hx,ix;
	unsigned lx;
	hx = __FDLIBM_HI(x);		/* high word */
	lx = __FDLIBM_LO(x);		/* low word */
	ix = hx&0x7fffffff;
